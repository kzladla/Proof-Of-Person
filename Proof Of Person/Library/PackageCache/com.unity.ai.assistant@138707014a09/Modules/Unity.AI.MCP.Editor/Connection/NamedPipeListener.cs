...
namespace Unity.AI.MCP.Editor.Connection
{
    ...
    /// Uses P/Invoke fallback for Unity runtimes that don't fully implement PipeSecurity API
    /// </summary>
...
#pragma warning disable CS1998 // Async method lacks 'await' operators (only awaits on Windows platform)
        public async Task<IConnectionTransport> AcceptClientAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // Try to create pipe with managed PipeSecurity first
                var pipeSecurity = CreateSecurePipeSecurity();

                if (pipeSecurity != null)
                {
                    // Managed API available - use it
                ...
                {
                    // Managed PipeSecurity API not available - try P/Invoke fallback
                    ...
                    // If P/Invoke also failed, fall back to pipe without ACL
...
#pragma warning restore CS1998
        ...
        /// <summary>
        /// Create PipeSecurity that allows only the current user to access the pipe
        /// Returns null if PipeSecurity API is not available (Unity runtime limitation)
        /// </summary>
        static PipeSecurity CreateSecurePipeSecurity()
        {
            try
            {
                ...
                using (WindowsIdentity currentIdentity = WindowsIdentity.GetCurrent())
                {
                    if (currentIdentity?.User != null)
                    {
                        ...
                        // Explicitly deny Everyone (World SID) to prevent other users
                        var worldSid = new SecurityIdentifier(WellKnownSidType.WorldSid, null);
                        var denyRule = new PipeAccessRule(
                            worldSid,
                            PipeAccessRights.FullControl,
                            AccessControlType.Deny);
                        pipeSecurity.AddAccessRule(denyRule);

                        McpLog.Log($"Created secure pipe ACL for user: {currentIdentity.Name}");
                        return pipeSecurity;
                    }
                    else
                    {
                        McpLog.Warning("Failed to get current user identity for pipe security");
                        return null;
                    }
                }
            }
            ...
            {
                // PipeSecurity API not implemented in this Unity runtime - caller will use P/Invoke fallback
                return null;
            }
            catch (Exception ex)
            {
                McpLog.Warning($"Failed to create secure pipe ACL: {ex.Message}");
                return null;
            }
        }
        ...
        /// This is used as fallback when the managed PipeSecurity API is not available.
        /// Creates pipe with owner-only ACL using SDDL.
        /// </summary>

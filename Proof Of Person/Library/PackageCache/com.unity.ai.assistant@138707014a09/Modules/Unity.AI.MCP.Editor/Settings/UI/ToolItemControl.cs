...
namespace Unity.AI.MCP.Editor.Settings.UI
{
    class ToolItemControl : VisualElement
    {
        ...
        public McpToolInfo ToolInfo { get; private set; }

        public ToolItemControl(McpToolInfo toolInfo)
        {
            ToolInfo = toolInfo;
            LoadTemplate();
            InitializeElements();
        ...
        void SetupTool()
        {
            m_NameLabel.text = ToolInfo.name;
            m_DescriptionLabel.text = ToolInfo.description ?? "";

            var settings = MCPSettingsManager.Settings;
            m_Checkbox.value = settings.IsToolEnabled(ToolInfo.name);
            ...
            // Handle clicking on the header (but not on checkbox or button)
        ...
        {
            var settings = MCPSettingsManager.Settings;
            settings.SetToolEnabled(ToolInfo.name, enabled);

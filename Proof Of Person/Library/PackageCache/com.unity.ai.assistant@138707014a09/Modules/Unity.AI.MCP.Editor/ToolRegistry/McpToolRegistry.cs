...
namespace Unity.AI.MCP.Editor.ToolRegistry
{
    ...
    static class McpToolRegistry
    {
        ...
        /// </remarks>
        internal static event Action<ToolChangeEventArgs> ToolsChanged;

        /// <summary>
        /// Event arguments for tool changes.
        /// Provides information about what changed in the tool registry.
        /// </summary>
        internal class ToolChangeEventArgs
        {
            /// <summary>
        ...
        public static void RefreshTools()
        {
            ...
            try
            {
                ...
                ToolsChanged?.Invoke(new ToolChangeEventArgs(ToolChangeType.Refreshed, null));
            }
            catch (Exception ex)
            {
                Debug.LogError($"[McpToolRegistry] Failed to discover tools: {ex.Message}\n{ex.StackTrace}");
            }
        }
        ...
        public static async Task<object> ExecuteToolAsync(string toolName, JObject parameters)
        {
            ...
            catch (TargetInvocationException ex)
            {
                ...
                McpLog.Error($"[McpToolRegistry] Error executing tool '{toolName}': {actualException.Message}");
                throw actualException;
            }
            catch (Exception ex)
            {
                McpLog.Error($"[McpToolRegistry] Error executing tool '{toolName}': {ex.Message}");
                throw;
            }
        }
        ...
        public static McpToolInfo[] GetAvailableTools(bool ignoreEnabledState = false)
        {
            ...
            foreach (var handler in k_Tools.Values)
            {
                if (ignoreEnabledState || settings.IsToolEnabled(handler.Attribute.Name))
                {
        ...
        public static void RegisterTool(string toolName, IUnityMcpTool toolInstance, string description = null) =>
            RegisterToolHandler(toolName,
                toolInstance,
                new ClassToolHandler(
                    toolInstance,
                    new McpToolAttribute(toolName, description ?? $"Dynamically registered tool: {toolName}")));
        ...
        /// <exception cref="ArgumentException">Thrown if toolName is null or empty</exception>
        /// <exception cref="ArgumentNullException">Thrown if toolInstance is null</exception>
        public static void RegisterTool<TParams>(string toolName, IUnityMcpTool<TParams> toolInstance, string description = null)
            where TParams : class =>
            RegisterToolHandler(toolName,
                toolInstance,
                new GenericClassToolHandler(
                    toolInstance,
                    new McpToolAttribute(toolName, description ?? $"Dynamically registered typed tool: {toolName}"),
                    typeof(TParams)));
        ...
        static void RegisterToolHandler(string toolName, object toolInstance, IToolHandler handler)
        {
            ...
            bool isUpdate = k_Tools.ContainsKey(toolName);
            k_Tools[toolName] = handler;

            var changeType = isUpdate ? ToolChangeType.Updated : ToolChangeType.Added;
            ToolsChanged?.Invoke(new ToolChangeEventArgs(changeType, toolName, handler));

            McpLog.Log($"[McpToolRegistry] {(isUpdate ? "Updated" : "Registered")} tool '{toolName}'");
        }
        ...
        public static bool UnregisterTool(string toolName)
        {
            ...
            if (k_Tools.Remove(toolName))
            {
                ToolsChanged?.Invoke(new ToolChangeEventArgs(ToolChangeType.Removed, toolName));
                McpLog.Log($"[McpToolRegistry] Unregistered tool '{toolName}'");
                return true;
            }
        ...
        static void DiscoverTools()
        {
            ...
            foreach (var method in toolMethods)
            {
                try
                {
                    ...
                    {
                        Debug.LogWarning($"[McpToolRegistry] Skipping invalid tool method: {method.DeclaringType?.Name}.{method.Name}. " +
                    ...
                    {
                        Debug.LogWarning($"[McpToolRegistry] Duplicate tool name '{toolAttribute.Name}' found. " +
                ...
                {
                    Debug.LogError($"[McpToolRegistry] Failed to register tool from method {method.DeclaringType?.Name}.{method.Name}: {ex.Message}");
                }
            }
            ...
            foreach (var type in toolClasses)
            {
                try
                {
                    ...
                    {
                        Debug.LogWarning($"[McpToolRegistry] Class {type.Name} must have a parameterless constructor to be used as MCP tool.");
                        continue;
                    }
                    ...
                    {
                        Debug.LogWarning($"[McpToolRegistry] Class {type.Name} has [McpTool] attribute but doesn't implement IUnityMcpTool or IUnityMcpTool<T> interface.");
                        continue;
                    }
                ...
                {
                    Debug.LogError($"[McpToolRegistry] Failed to register class-based tool from {type.Name}: {ex.Message}");
                }
            }

            // Validate schema methods have corresponding tools
            foreach (var kvp in schemaMethods)
            {
                if (!k_Tools.ContainsKey(kvp.Key))
                {
                    Debug.LogWarning($"[McpToolRegistry] Schema method for tool '{kvp.Key}' found but no corresponding tool method. " +
            ...
            foreach (var kvp in outputSchemaMethods)
            {
                if (!k_Tools.ContainsKey(kvp.Key))
                {
                    Debug.LogWarning($"[McpToolRegistry] Output schema method for tool '{kvp.Key}' found but no corresponding tool method. " +
                                   $"Method: {kvp.Value.DeclaringType?.Name}.{kvp.Value.Name}");
                }
            }
        }
        ...
        static bool IsDebugEnabled()
        {
            try
            {
                return MCPSettingsManager.Settings.debugLogsEnabled;
            }
            catch
            {
                return false;
        ...
        {
            ToolsChanged?.Invoke(new ToolChangeEventArgs(ToolChangeType.Updated, toolName));
        }
    }
}

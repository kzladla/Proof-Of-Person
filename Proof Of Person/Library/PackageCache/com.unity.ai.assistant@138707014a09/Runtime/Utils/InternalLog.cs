...
namespace Unity.AI.Assistant.Utils
{
    ...
    static class InternalLog
    {
        ...
        static void LogInternal(LogType logType, object message)
        {
            object logContent = message;

#if UNITY_EDITOR
            if (!MainThread.IsMainThread)
            {
                // Capture stack trace before dispatching to preserve call context
                var stackTrace = new StackTrace(2, true);
                if (message is Exception ex)
                    logContent = new Exception($"{ex.Message}\n[Original stack]:\n{stackTrace}", ex);
                else
                    logContent = $"{message}\n[Original stack]:\n{stackTrace}";
            }
#endif
            ...
            {
                if (logType == LogType.Exception && logContent is Exception ex)
        ...
        public static void LogException(Exception exception, LogFilter filter = LogFilter.General)
        {
            ...
            LogInternal(LogType.Exception, exception);
        }

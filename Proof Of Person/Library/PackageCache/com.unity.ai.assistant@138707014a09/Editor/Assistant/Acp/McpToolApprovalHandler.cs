...
namespace Unity.AI.Assistant.Editor.Acp
{
    ...
    /// 2. Bridge routes to RelayService.RequestMcpToolApprovalAsync
    /// 3. This handler shows permission UI via AcpSession.FireMcpPermissionRequest
    /// 4. User responds, AcpProvider calls CompleteMcpApproval
    ...
    static class McpToolApprovalHandler
    {
        ...
        static readonly ConcurrentDictionary<string, TaskCompletionSource<RelayService.McpToolApprovalResponse>> s_PendingApprovals = new();

        /// <summary>
        /// Initialize the handler by registering with RelayService.
        /// Called from Bridge initialization.
        /// </summary>
        public static void Initialize()
        {
            ...
            RelayService.Instance.OnMcpToolApprovalRequest = HandleApprovalRequest;
        }

        /// <summary>
        /// Handle MCP tool approval requests from Bridge via RelayService.
        /// Routes to the appropriate AcpSession for permission UI.
        /// </summary>
        static async Task<RelayService.McpToolApprovalResponse> HandleApprovalRequest(
            RelayService.McpToolApprovalRequest request)
        {
            // Find the AcpSession by sessionId
            var session = FindSessionByChannelId(request.SessionId);
            if (session == null)
            {
                return new RelayService.McpToolApprovalResponse(true, "No active session (auto-approved)");
            }
            ...
            if (mode == "full-auto")
            {
                return new RelayService.McpToolApprovalResponse(true, "Auto-approved (full-auto mode)");
            }

            // For other modes, show permission UI
            var tcs = new TaskCompletionSource<RelayService.McpToolApprovalResponse>();
            ...
            try
            {
                ...
                if (completedTask == timeoutTask)
                {
                    ...
                    return new RelayService.McpToolApprovalResponse(false, "Approval timed out");
                }
        ...
        public static void Complete(string toolCallId, bool approved, string reason = null, bool alwaysAllow = false)
        {
            ...
            {
                tcs.TrySetResult(new RelayService.McpToolApprovalResponse(approved, reason, alwaysAllow));
            }
        }

...
namespace Unity.AI.Assistant.Editor.Mcp.Manager
{
    ...
    {
        const int k_SecondsBeforeRelayTimeout = 60;
        ...
        public async Task Initialize()
        {
            ...
            bool initializeSuccess = await Utils.TaskUtils.AwaitCondition(
                () => RelayService.Instance.State.Status == RelayStatus.Running, cancellationTokenSource.Token);
            ...
            RelayClient = new UnityMcpHttpClient($"http://localhost:{RelayService.Instance.McpClientPort}");
            await LoadConfig();
        }
        ...
        {
            // Filter out hidden servers (names starting with ~)
            var serversToAdd = m_CurrentConfiguration.Servers
        ...
        {
            foreach (var server in m_CurrentConfiguration.Servers)

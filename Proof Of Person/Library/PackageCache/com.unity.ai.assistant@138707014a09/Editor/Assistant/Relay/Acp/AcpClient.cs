...
namespace Unity.Relay.Editor.Acp
{
    ...
    class AcpClient : IDisposable
    {
        ...
        void SubscribeToRelayEvents()
        {
            ...
            // If we were created after the relay was already connected, request providers now.
            if (IsConnected)
            {
        ...
        /// <param name="env">Optional environment variables to use for validation.</param>
        /// <returns>True if the request was sent successfully.</returns>
        public async Task<bool> ValidateExecutableAsync(string agentType, Dictionary<string, string> env = null)
        {
            if (!IsConnected)
            {
                return false;
        ...
        public async Task<bool> StartSessionAsync(AcpSessionConfig config)
        {
            ...
            try
            {
                var message = new
                {
                    ...
                    env = config.Env,
                    secureEnvVarNames = config.SecureEnvVarNames,
